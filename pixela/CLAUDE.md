...
- **The Supabase database is gone.** As of 2026-09-01 the connection string in `.env.local` fails with `FATAL: (ENOTFOUND) tenant/user postgres.seyonpubmzfqvpntdifx not found` — the pooler does not know that project. Free-tier Supabase pauses on inactivity and eventually removes the project. Everything that touches Prisma is therefore broken: login, register, profile, favorites, library and reviews. The TMDB catalog is unaffected, so the app still renders. Provision a new database and update `DATABASE_URL` (and Vercel's env var) before expecting user features to work. **Corollary that bit us once already:** a server component must never let a database failure decide whether a page exists — see `getMediaReviewsSafe`.
...
- **A detail page fetches its own database rows server-side.** `MediaPage` used to pull reviews in a `useEffect` on mount: one Edge Request plus one function invocation per view of a film, anonymous visitors included, on a page that is otherwise fully ISR-cached. The pages now call `getMediaReviewsSafe`, and the review mutation routes call `revalidatePath` so the cached HTML refreshes when someone posts. Use the **`Safe`** variant in pages and the plain one in route handlers: a page must render even when the database is down, an API must report the failure.
- **`/api/auth/session` is not asked for anonymous visitors.** `src/proxy.ts` maintains a non-httpOnly `pixela.has-session` hint cookie, and `providers.tsx` passes `session={null}` when it is absent, which skips the fetch entirely (verified in `node_modules/next-auth/react.js`: `_getSession` only hits the network when `__NEXTAUTH._session === undefined`). The hint carries no token and is not an authorization mechanism — forging it only buys you a request that returns `null`. Keep the cookie name in sync between those two files.
